...
export default function ProjectionPage() {
  ...
  useEffect(() => {
    if (selectedComptes.length && targetDate) compute()
  }, [selectedComptes, targetDate])
  ...
  async function compute() {
    ...
    // Load all transactions for selected accounts (past + future up to target date)
    ...
    // Build day-by-day chart from today to targetDate
    const days = []
    let cursor = new Date()
    ...
    // For each account, compute running balance
    const runningBalances = {}
    const filteredComptes = comptes.filter(c => selectedComptes.includes(c.id))

    for (const c of filteredComptes) {
      // Start from initial balance + all past transactions up to yesterday
      const pastTxns = txns.filter(t => t.compte_id === c.id && t.date < today)
      const base = parseFloat(c.solde_initial) + pastTxns.reduce((s, t) => s + parseFloat(t.montant), 0)
      runningBalances[c.id] = { base, daily: {} }
    }
    ...
    const chart = days.map(day => {
      ...
      for (const c of filteredComptes) {
        ...
        const daySum = dayTxns.reduce((s, t) => s + parseFloat(t.montant), 0)
        runningBalances[c.id].base += daySum
    ...
    // Final projected balances
    const finalBal = {}
    for (const c of filteredComptes) {
      finalBal[c.id] = runningBalances[c.id].base
    }
          ...
          <div className="flex gap-2">
            {['30', '90', '180', '365'].map(d => (
              <button
                key={d}
                type="button"
                onClick={() => setTargetDate(format(addDays(new Date(), parseInt(d)), 'yyyy-MM-dd'))}
            ...
            <LineChart data={chartData} margin={{ right: 20 }}>
              ...
              {allLines.map((name, i) => (
                <Line
                  ...
                  strokeWidth={name === 'Total' ? 2.5 : 1.5}
                  dot={false}
            ...
            <tbody className="divide-y divide-gray-50">
              {comptes.filter(c => selectedComptes.includes(c.id)).map(c => {
                ...
                return (
                  <tr key={c.id}>
                    ...
                    <td className={`py-2 text-right font-semibold ${projected >= 0 ? 'text-gray-900' : 'text-red-500'}`}>
                      {fmt(projected)}
                    </td>
              ...
              {selectedComptes.length > 1 && (
                <tr className="border-t-2 border-gray-200 font-bold">
                  ...
                  <td className="pt-2 text-right">{fmt(chartData[0] ? Object.values(chartData[0]).filter(v => typeof v === 'number').slice(0, -1).reduce((a, b) => a + b, 0) : 0)}</td>
                  <td className="pt-2 text-right">{fmt(Object.values(projectedBalances).reduce((a, b) => a + b, 0))}</td>
                  <td className="pt-2 text-right text-emerald-600">
                    {fmt(Object.values(projectedBalances).reduce((a, b) => a + b, 0) - (chartData[0] ? comptes.filter(c => selectedComptes.includes(c.id)).reduce((s, c) => s + (chartData[0][c.nom] ?? 0), 0) : 0))}
                  </td>
                </tr>
              )}
            </tbody>
