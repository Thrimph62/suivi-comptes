    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowNewParent(true)} className="btn-secondary text-sm">
            + Catégorie
          </button>
          <button onClick={() => setShowModal(true)} className="btn-primary flex items-center gap-2 text-sm">
            <Plus size={16} /> Sous-catégorie
          </button>
        </div>
      </div>
      ...
      {parents.length === 0 ? (
        <div className="card text-center py-10">
          ...
          <button onClick={() => setShowNewParent(true)} className="btn-primary">
            ...
            return (
              <div key={parent} className="card p-0 overflow-hidden">
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={e => { e.stopPropagation(); setForm({ parent, sous_categorie: '' }); setShowModal(true) }}
                    ...
                    {parentOnly && (
                      <button
                        onClick={e => { e.stopPropagation(); setDeleteTarget(parentOnly) }}
        ...
        <Modal title="Nouvelle catégorie principale" onClose={() => setShowNewParent(false)} size="sm">
          <div className="space-y-3">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button className="btn-primary" onClick={addParent} disabled={saving || !newParent}>
                {saving ? 'Ajout…' : 'Ajouter'}
              </button>
            </div>
          </div>
      ...
      {deleteTarget && (
        <Modal title="Supprimer" onClose={() => setDeleteTarget(null)} size="sm">
          <p className="text-gray-600 mb-4">
            Supprimer <strong>{deleteTarget.sous_categorie || deleteTarget.parent}</strong> ?
